...
class AgeTransformationNode:
    ...
    def transform_age(self, input_image, target_age):
        ...
        global net
        global transform
        if predictor is None or net is None or transform is None:
            load_model()
        """
        ...
        result_tensor = torch.from_numpy(np.array(result_image).astype(np.float32) / 255.0).unsqueeze(
            0)  # Add batch dimension if not present
...
NODE_CLASS_MAPPINGS = {
    ...
    "AgeOffloadModelNode": AgeOffloadModelNode
}
